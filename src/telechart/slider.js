...
export class SliderModule extends Module {
  ...
  render(state: State) {
    ...
    const leftPercent = (state.slice.start * 100).toFixed(3) + "%";
    const rightPercent = (state.slice.end * 100).toFixed(3) + "%";
    const invRightPercent = ((1 - state.slice.end) * 100).toFixed(3) + "%";
    const width =
      ((state.slice.end - state.slice.start) * 100).toFixed(3) + "%";
